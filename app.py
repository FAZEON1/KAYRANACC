...
if sayfa == "📊 Dashboard":
    ...
    nakit_bg    = "#1a472a" if hafta_sonu_tl >= 0 else "#7f1d1d"
    nakit_renk  = "#4ade80" if hafta_sonu_tl >= 0 else "#f87171"
    ...
    <div class="kart-grid">
      ...
      <div class="kart">
        ...
        <div class="kart-deger" style="color:#818CF8">${fmt(usd_toplam)}</div>
        <div class="kart-alt">≈ ₺{fmt(usd_toplam * kur)}</div>
      </div>

      <div class="kart">
        <div class="kart-label">İlerleme</div>
        <div class="kart-deger" style="color:#F1F5F9">{odendi_cnt} <span style="font-size:15px;color:#475569">/ {len(odemeler)}</span></div>
        <div class="progress-bar-bg">
          <div class="progress-bar-fill" style="width:{ilerleme_pct}%"></div>
        </div>
      ...
      <div class="kart" style="background:{nakit_bg};border-color:rgba(255,255,255,0.1)">
        <div class="kart-label" style="color:rgba(255,255,255,0.5)">{nakit_label}</div>
