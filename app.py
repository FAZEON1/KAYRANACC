...
.baslik { font-size: 26px; font-weight: 800; color: #0B1437; margin-bottom: 4px; }
.alt-baslik { font-size: 13px; color: #757575; margin-bottom: 20px; }
...
.tag-mavi    { background:#BBDEFB; color:#1565C0; padding:3px 10px; border-radius:12px; font-size:12px; font-weight:600; }
.tag-gri     { background:#F0F0F0; color:#616161; padding:3px 10px; border-radius:12px; font-size:12px; }

.uyari-box { background:#FFF3E0; border-left:4px solid #FF6F00; padding:10px 16px; border-radius:4px; margin:6px 0; font-size:13px; }
.info-box  { background:#E3F2FD; border-left:4px solid #1565C0; padding:10px 16px; border-radius:4px; margin:6px 0; font-size:13px; }
.ok-box    { background:#E8F5E9; border-left:4px solid #2E7D32; padding:10px 16px; border-radius:4px; margin:6px 0; font-size:13px; }
.alarm-box { background:#FFEBEE; border-left:4px solid #C62828; padding:10px 16px; border-radius:4px; margin:6px 0; font-size:13px; }

section[data-testid="stSidebar"] { background: #0B1437 !important; }
...
# ── SIDEBAR ──────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## 💳 KAYRANACC")
    aktif_kullanici = st.session_state.get("aktif_kullanici", "")
    if aktif_kullanici:
        st.markdown(f"""
        <div style="background:#162050;border-radius:8px;padding:8px 12px;margin-bottom:8px;">
    ...
    if hafta:
        st.markdown(f"""
        <div style="background:#162050;border-radius:8px;padding:8px 12px;margin-bottom:8px;">
            <span style="color:#90CAF9;font-size:11px;">📅 Aktif Hafta</span><br>
            <span style="color:white;font-weight:bold;font-size:12px;">{hafta['hafta_adi']}</span>
        </div>
        """, unsafe_allow_html=True)
...
if sayfa == "📊 Dashboard":
    ...
    m1.metric("📋 Toplam TL", f"₺{fmt(tl_toplam)}")
    m2.metric("💵 Toplam USD", f"${fmt(usd_toplam)}")
    m3.metric("✅ Ödendi", f"{odendi_cnt}/{len(odemeler)}")
    m4.metric("⏳ Bekleyen TL", f"₺{fmt(bekleyen_tl)}")
    m5.metric("🏦 Banka TL", f"₺{fmt(banka_tl)}")

    # Nakit durum
    hafta_sonu_tl = banka_tl - bekleyen_tl - (bekleyen_usd * kur)
    if hafta_sonu_tl < 0:
        st.markdown(f'<div class="alarm-box">⚠️ <b>NAKİT AÇIĞI:</b> Mevcut TL bakiye, bekleyen ödemeler için yetersiz! Tahmini açık: <b>₺{fmt(abs(hafta_sonu_tl))}</b></div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="ok-box">✅ <b>Nakit Durumu Normal:</b> Tüm ödemeler karşılandıktan sonra tahmini kalan: <b>₺{fmt(hafta_sonu_tl)}</b></div>', unsafe_allow_html=True)
    ...
    with col2:
        ...
        fig2.update_layout(height=300, margin=dict(t=20, b=20), paper_bgcolor="white")
    ...
    for day in sorted(by_day.keys()):
        ...
        tablo_rows.append({
            ...
            "Tutar TL (₺)": gun_tl or None,
            "Tutar USD ($)": gun_usd or None,
...
# ════════════════════════════════════════════════════════════════════
    ...
    m2.metric("Toplam USD", f"${fmt(usd_toplam)}")
    m3.metric("Ödendi", f"{odendi_cnt}/{len(odemeler)}")
    m4.metric("Kalan TL", f"₺{fmt(tl_toplam - odendi_tl)}")
...
# ════════════════════════════════════════════════════════════════════
    ...
    m1.metric("Ödenen TL", f"₺{fmt(tl_top)}")
    m2.metric("Ödenen USD", f"${fmt(usd_top)}")
    m3.metric("Ödeme Adedi", len(odenenler))
